...
const GoogleButton = ({ onSuccess, onError, loading, disabled, text = "Daftar dengan Google" }) => {
...
const RegisterPageContent = () => {
    ...
    const handleGoogleSuccess = async (response) => {
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error(data.error || 'Registrasi Google gagal');
            }
    ...
    return (
        <div className="min-h-screen bg-[#0F172A] flex items-center justify-center p-6 relative overflow-hidden">
            ...
            <div className="fixed inset-0 overflow-hidden pointer-events-none">
                <div className="absolute top-[-10%] right-[-10%] w-[50%] h-[50%] bg-[#06B6D4]/10 blur-[160px] rounded-full animate-pulse"></div>
                <div className="absolute bottom-[-10%] left-[-10%] w-[50%] h-[50%] bg-[#6366F1]/10 blur-[160px] rounded-full"></div>
            </div>
            ...
            <div className="w-full max-w-md relative z-10 animate-fade-in">
                ...
                <div className="bg-[#1E293B]/60 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl">
                    <div className="text-center mb-8">
                        <h1 className="text-2xl font-black text-white mb-2">Mulai Perjalananmu</h1>
                        <p className="text-slate-400 text-sm">Daftar untuk mengakses panduan spiritual personal</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-sm font-bold text-slate-400 mb-2">Nama Lengkap</label>
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-400 mb-2">Konfirmasi Password</label>
                            <div className="relative">
                                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
                                <input
                                    type={showPassword ? 'text' : 'password'}
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-gradient-to-r from-[#6366F1] to-[#06B6D4] text-white font-black py-4 rounded-xl transition-all shadow-lg shadow-[#6366F1]/30 hover:brightness-110 disabled:opacity-50 uppercase tracking-widest text-xs mt-2"
                        >
                    ...
                    {/* Google Register Button - Custom styled */}
                    <GoogleButton
